...
.context-guard/
  ...
  │       ├── objective.md
  │       ├── snapshot.md
  │       ├── tasks.md           (manual or generated by tasks skill)
...
4. **Task generation**: If reference documents (SPECs, ADRs, design docs) exist, populate `reference_docs` in the manifest and invoke the `tasks` skill (see `references/tasks_list.md`) to generate `tasks.md`. Otherwise, generate `tasks.md` manually with a simple task list.
...
1. Read `snapshot.md`, `objective.md`, and `tasks.md` to rehydrate context.
2. No session lock is needed for read-only rehydration.

### STEP 3 — AUTONOMOUS DISCOVERY & EXECUTION

Work on tasks from `tasks.md` using **per-task locking**:
