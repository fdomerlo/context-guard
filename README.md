...
### Structured planning with `cg plan`

For requirements that need multi-step or phased decomposition, `cg plan` structures the requirement directly into objectives, phases (`F1`, `F2`, …), tasks, and acceptance criteria:

```bash
# (1) initialize the repository agent contract and git hooks
cg init

# (2) structure a requirement into a multi-phase plan
cg plan "Add OAuth2 authentication"

# (3) a human reviews objective.md and tasks.md, then approves
cg approve

# (4) begin EXECUTE and claim tasks atomically
cg begin --phase EXECUTE
cg next-task

# (5) verify acceptance criteria and test suite
cg verify

# (6) commit and advance to the next phase (or archive)
...
Per project, run `cg init` once to scaffold the agent contract (`AGENTS.md`),
configure harness rules, install git hooks, and materialise the phase files.
`cg new` also materialises any missing phase files on demand.
...
context-guard unifies repository discipline, agent contracts, and transactional phase enforcement into a single workflow:

1. **Scaffold the repository contract (`cg init`)**: Idempotently generates `AGENTS.md`, harness rules (`CLAUDE.md`, `.cursorrules`, `.agent/rules/`), Git hooks (`.githooks/commit-msg` enforcing Conventional Commits and `.githooks/pre-commit`), and activates `core.hooksPath`.
2. **Deconstruct requirements into structured plans (`cg plan`)**: Reads requirement prompts or specs, structuring objectives, specifications, multi-phase DAGs (F1, F2...), tasks (`tasks.md`), and acceptance criteria directly into `manifest.json`.
3. **Approve human gate (`cg approve`)**: Enforces human sign-off before entering `EXECUTE` for each phase.
4. **Execute tasks atomically (`cg begin`, `cg next-task`, `cg release-task`)**: Claim and work through tasks one by one with atomic locking.
5. **Verify and audit (`cg verify`)**: Verifies test suite execution, checks task and acceptance criteria completion, updates audit reports (`verify-report.md`, `review-report.md`), and transitions to the next phase or archive.
...
| Command | Purpose |
|---|---|
| `cg init [--strict] [--no-hooks]` | Scaffold repository agent contract (`AGENTS.md`), host rules, and git hooks |
| `cg plan <requirement> [--from-file <path>]` | Deconstruct requirement into structured plan, multi-phase DAG, and tasks |
...
| `cg verify [--fix] --context <path>` | Verify phase tasks, acceptance criteria, and update audit reports |
...
previously generated by [disciplined-scaffold](https://github.com/fdomerlo/disciplined-scaffold)
after talking a project through — `cg new --from-plan` materialises it instead of retyping:

```bash
cg new redis --from-plan PLAN-3.md
```

Note: Planning and scaffolding capabilities formerly in `disciplined-scaffold`
are now natively unified into `context-guard` via `cg init` and `cg plan`.
The `--from-plan` flag remains fully supported for backward compatibility
when migrating existing projects or importing legacy `PLAN-N.md` documents.
...
The full unified flow: `cg init` scaffolds the repo contract → `cg plan` decomposes
the requirement → `cg approve` reviews each phase → `cg begin` and `cg next-task` execute →
`cg verify` audits → `cg commit` advances.

## Pre-commit hook

Git hooks are installed automatically by `cg init` or activated once per clone with
`git config core.hooksPath .githooks`:

- **`.githooks/commit-msg`**: Rejects commit messages that do not follow Conventional
  Commits (`feat`, `fix`, `docs`, `refactor`, `test`, `chore`, `release`). Bypass with
...
[disciplined-scaffold](https://github.com/fdomerlo/disciplined-scaffold) was
the lighter step before this one: markdown-only phase discipline, no
transactional state. Its capabilities (agent contract, scaffolding, conventional
commit hooks, structured multi-phase planning) are now natively integrated
into context-guard (`cg init` and `cg plan`). For projects with existing plans,
`cg new --from-plan` reads legacy `PLAN-N.md` files — see [From a phased plan](#from-a-phased-plan).
