...
# Instala la skill en la ruta global ~/.gemini/skills/ y el motor en bin/
# ============================================================================
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    *) echo "Usage: install.sh --target antigravity | --uninstall"; exit 1 ;;
  esac
done
...
if [ "$TARGET" == "antigravity" ]; then
    ...
    echo "Por favor, especifica el target: bash scripts/install.sh --target antigravity"
fi